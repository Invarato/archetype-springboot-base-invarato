...
#[[##]]# Empezando
1. Compilar y ejecutar Unit tests (Ejecuta solo `*Test.java`):
   ```shell
   mvn clean test
   ```
2. Compilar y ejecutar Integration tests (Ejecuta solo `*IT.java`; https://docs.spring.io/spring-boot/maven-plugin/integration-tests.html):
   ```shell
   mvn clean verify
   ```
3. Ejecutar la aplicación:
   ...
   mvn spring-boot:run
   ```
4. Endpoint de ejemplo (puede variar según plantilla):
